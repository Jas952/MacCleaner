...
Дополнение: RAM utilization не является thermal reading и не влияет на высоту поверхности. Battery показывается как единый pack без неподтверждённого cell count. Конкретный power connector разрешается только через model-specific IORegistry mapping: для проверенного Mac15,6 порядок контроллеров — USB-C 1, USB-C 2, MagSafe 3, USB-C 3; неизвестные модели получают generic-подпись. Одновременно рисуется только активный кабель. Component inspection выполняет top-down camera transition до скрытия mesh, а zoom slider раскрывается из отдельной icon-only кнопки. На Mac15,6 дополнительная icon-only кнопка меняет только визуальную основу между схемой и фотореалистичной текстурой через animatable crossfade; единое поле температур и все интерактивные overlay остаются неизменными. Геометрия обеих основ принимает натуральное отношение сторон texture asset вместо деформации изображения под квадратную схему; projector ограничивает общий масштаб по фактическому повороту, чтобы более широкий корпус оставался внутри Canvas. На других моделях кнопка скрыта, чтобы модель-специфичная текстура не выдавала предположение за точную геометрию.
...
## Thermal Surface: источники отдельно от интерполяции

Статус: принято

Решение: thermal mesh принимает только реальные readings с исходными идентификаторами. Не создавать отсутствующие GPU/SSD/airflow значения из температуры соседнего компонента. HID tdie/tdev сохраняют channel identity; недокументированные индексы не интерпретируются как точные номера P/E-ядер. Схема и фотографическая основа остаются на прежних координатах. Пространственная интерполяция показывает распространение влияния датчиков и помечается `Interpolated`; подсказки компонента и ближайшего датчика используют исходный live sample, не промежуточное значение анимации.

Обоснование: данные аппаратного канала и температура произвольной точки пространства — разные сущности. Минимальный реальный sample служит визуальным baseline, без заявления о температуре воздуха. Нераспознанное расположение нельзя исправить выдуманной thermal-зоной. Численные интервалы точности не заявляются без калибровки.

Проверки: regression-тесты на отсутствие подставных датчиков, стабильность ID и координат, сохранение температуры в опорных точках, ограниченность интерполяции, исходные значения во время анимации, фильтрацию NaN/дубликатов и шкалу выше 95°C.
