...
Menu bar использует существующий `SystemMonitor`, поэтому включение CPU/RAM/GPU/temperature/battery gauges не создаёт параллельный sampler. `StatusBarController` владеет AppKit `NSStatusItem` и формирует attributed label в одном из двух сохранённых режимов. `Battery` показывает подписанные вертикальные indicators: заполнение снизу вверх передаёт нагрузку/нагрев, semantic green/orange/red — severity, temperature сохраняет термометр, а compact marker (`%/C`, `%/G`, `%/°`, `C/F`, `%/clock`) обозначает выбранный формат и использует тот же semantic color вместо малоконтрастного secondary label. `Values` вместо battery выводит рядом с `CPU`, `RAM`, `GPU`, `TEMP`, `BAT` фактическое процентное или числовое значение в выбранном формате. В обоих режимах точные values остаются в accessibility label, tooltip и SwiftUI `MenuBarPopover`. Settings хранит стиль, состав, порядок и форматы gauges; включённые строки являются настоящими draggable tiles, поэтому системный preview поднимает плитку в исходной геометрии, а `dropEntered` анимированно раздвигает соседние элементы до отпускания. Выключенные строки остаются ниже как неактивные плитки. Когда все gauges выключены, status item показывает фактический bundle icon MacCleaner, а не отдельный SF Symbol.
