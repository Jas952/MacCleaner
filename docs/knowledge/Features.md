...
- Tools polish: Homebrew secondary actions use contrast-safe buttons and aligned maintenance-policy icons; Color Picker exposes an inline HEX copy control; File Reader is marked beta and unavailable in the active Tools sidebar; Physical Maintenance and Speaker Test use compact no-scroll layouts; Device Health metric explanations use native hover tooltips. Settings companion copy/download controls and Menu Bar quick-tool checkboxes use consistent alignment. Drop Shelf configuration in Tools contains settings only; the actual drop target remains in the floating Shelf window.
...
- Модульный menu bar: выбираемые CPU/RAM/GPU/temperature/battery gauges представлены draggable tiles в Settings. Каждый gauge независимо выбирает `Battery / Values` и формат; прежний общий стиль мигрирует без сброса настроек. В реальном status item gauges без разделителей собираются попарно в компактные вертикальные колонки: CPU над RAM, следующая пара рядом. При отключении всех gauges остаётся настоящий bundle icon MacCleaner.
- Новый System popover объединяет прежние Overview и Details и удаляет отдельный Watch-score. Он переиспользует те же 166-pt Dashboard-карточки CPU, Memory, Disk, Network, Graphics и Battery, включая графики и кольца батарей Mac/iPhone/AirPods. System/Tools представлены icon-only controls возле иконки приложения. В Edit карточка поднимается из фактической точки захвата на handle, high-priority drag ручки не перехватывается ScrollView, соседние карточки пружинно раздвигаются, а при отпускании карточка пружинно доезжает до slot и без вспышки сливается с основной карточкой. Стабильный Reduce-Motion-aware wiggle гарантированно прекращается после Done без смены view hierarchy; нижняя область возвращает скрытые карточки. Адаптивная material-подложка с мягкими цветовыми tint-слоями, короткой alpha-mask краёв без тумана/свечения и coalesced обновления `SystemMonitor` уменьшают визуальную строгость и лаги без второго sampler.
- Header popover показывает кликабельную иконку MacCleaner, а footer содержит icon-only Settings и нативное overflow-меню для открытия приложения, окна About и завершения MacCleaner. Все controls имеют одинаковую 32-pt геометрию и показывают тонкую поверхность только при hover; overflow-chevron скрыт.
...
- 55 safety/policy XCTest-тестов, включая persistence-контракт порядка, удаления и восстановления menu bar dashboard-карточек и проверку валидности полного direct-drag порядка.
...
- [[Decisions]]
- [[Opportunities]]
- Fans: на Apple Silicon добавлен control UI с Auto и RPM-пресетами через отдельный XPC boundary. Фактическое управление выполняет `MacCleanerFanHelper`; до его установки доступна только честная ошибка helper unavailable. Helper поддерживает mode-key probing, bounded `Ftst` unlock, wake re-acquire и Auto fallback при disconnect/quit.
