...
- Menu bar → Graphs показывает историю текущих процессов в режимах CPU, Memory и Energy и отдельную изометрическую тепловую поверхность MacBook. Process history переключается иконками в углу графика между последними 30 минутами и четырьмя часами; архив timestamped samples очищается автоматически. В открытом popover данные поступают каждые 15 секунд, а в фоне сохраняется облегчённый process-only sample раз в пять минут. Первый полученный sample сразу показывается точками, второй начинает линии; принудительное получение процессов при открытии не теряется из-за параллельного monitor refresh. До шести процессов рисуются независимыми цветными линиями с лёгкой заливкой, разрывы cadence начинают новый сегмент, а render-path ограничен 180 точками. При переключении метрики или интервала история индексируется и подготавливается один раз, поэтому Canvas не повторяет фильтрацию архива для каждой точки. Кнопки метрик и режимов имеют hit-area на всю видимую плитку. Processes и Thermal surface удерживаются после первой загрузки для мгновенного возврата, но скрытая тепловая сцена прекращает кадровую анимацию. Над графиком остаётся одна полоса ведущего приложения с оригинальной иконкой и долей от показанных процессов; ось явно подписывает единицу. Hover показывает приложение, время и актуальное значение. Полный архив записывается на диск не чаще раза в минуту. Аргументы командной строки в историю не записываются. Поверхность использует реальные доступные SMC/HID sensors, высотой кодирует температуру и накладывает её на схему logic board, SSD, вентиляторов, аккумулятора, трекпада, верхнего бокового расположения динамиков и задней вентиляции для семейства MacBook Air/Pro. Модель вращается drag-жестом, масштабируется ползунком и колесом мыши, содержит оси X/Y/Z; reset возвращает одновременно базовый ракурс и 100% масштаба. Покадровая последовательность сначала возвращает камеру, затем плавно разделяет или соединяет слой компонентов и слой тепловых волн. Отдельный component-inspection скрывает поверхность и подсвечивает компонент под указателем с layout index, назначением и доступными live-показателями. При внешнем питании отображаются допустимые MagSafe 3 и USB-C power routes, текущий процент и charging pulse; публично недоступный физический active port не выдумывается. Ячейка mesh подсвечивается под указателем и показывает текущую интерполированную температуру и статус по непрерывной цветовой шкале. Apple Silicon RPM декодируется из SMC `flt `, Intel — из `fpe2`; при фактическом RPM выше нуля над каждым вентилятором движется объёмная система спиральных streamlines, частиц и проецируемых в 3D поперечных колец, а при 0 RPM движение не рисуется. Вся область использует адаптивный macOS material вместо фиксированного тёмного фона. Новые samples интерполируются без скачка геометрии; если конкретный sensor недоступен, соответствующая серия или зона не подменяется выдуманным значением.

- Стабилизация Process History: серия допускается на график только после нескольких samples и ранжируется по накопленному вкладу, поэтому краткий процесс или единичный пик не заменяет устойчивую линию. Пропуск одного-двух samples интерполируется, реальное появление/исчезновение получает короткий переход к нулю, а фрагменты короче минимальной серии не рисуются.

- Устойчивость архива Process History: повторяющиеся записи одного stable process identity объединяются как при новом замере, так и при загрузке старого JSON. CPU, память и число экземпляров суммируются, поэтому прежний архив не может закрыть приложение при переходе в Graphs.

- Быстрый старт Process History: процесс из самого свежего sample показывается сразу, даже если он ещё не прошёл порог стабильности на фоне длинного архива. Первый bucket рисуется точкой, следующий продолжает линию; пустой Canvas при уже заполненной summary-полосе не допускается.
- Разрывы Process History: общий перерыв collector до 15 минут на 30-минутном графике и до 30 минут на четырёхчасовом обозначается пунктирным мостом. Исчезновение отдельного процесса при продолжающемся сборе остаётся честным разрывом.
- Release-проверка `v1.0.8`: idle CPU Release-сборки не вырос относительно установленной `v1.0.7`; дополнительная память составила около 9,5 MB из-за bounded history и новых экранов. Скрытая Thermal Surface не поддерживает активную animation timeline.

- Масштаб Process History: CPU показывает долю от полной мощности всех логических ядер в диапазоне 0–100%, Memory — от 0 до физического объёма RAM текущего Mac, а Energy честно остаётся относительной activity-проекцией без выдуманных ватт. Линейная шкала включена по умолчанию, icon-only кнопка включает логарифмическую; вторая кнопка скрывает или возвращает процессы, чей пик ниже 1% системного предела. Четырёхчасовой режим агрегирует историю в 72 медианных интервала с лёгким сглаживанием, показывает до четырёх значимых линий без заливки и не дорисовывает падение к нулю при исчезновении процесса. Короткий период без collector samples соединяется тонким пунктиром, а не пустой полосой или выдуманными сплошными измерениями.

- Уточнение Thermal Surface: раскрываемый zoom-control живёт в той же колонке icon-only инструментов, открывается вертикально вниз от своей кнопки и не перекрывает вращаемую схему слева. Component inspection сначала плавно переводит камеру строго сверху и лишь затем скрывает mesh; возврат восстанавливает исходную перспективу. На Mac15,6 новая icon-only кнопка с плавным crossfade переключает нижнюю основу между уточнённой векторной схемой и модель-специфичной фотореалистичной текстурой; обе основы используют натуральную пропорцию корпуса `900:659`, а тепловая поверхность, управление, оси и легенда остаются общими. CPU Core и SoC Block readings создают отдельные пики внутри logic-board envelope, но не выдаются за публично недоступные физические координаты датчиков. RAM utilization не превращается в температуру. Battery показывается единым pack без неподтверждённого Apple количества ячеек. Для подтверждённой модели Mac15,6 активный IORegistry power controller сопоставляется с USB-C 1/2, MagSafe 3 или USB-C 3; на остальных моделях остаётся generic-подпись. Рисуется один фактический кабель, charging pulse и процент у разъёма; единственный внешний дисплей на питающем USB-C дополняет подпись системным именем и разрешением.
...
- Модульный menu bar: выбираемые CPU/RAM/GPU/temperature/battery gauges представлены draggable tiles в Settings. Каждый gauge независимо выбирает `Battery / Values` и формат; прежний общий стиль мигрирует без сброса настроек. В реальном status item gauges без разделителей собираются попарно в компактные вертикальные колонки: CPU над RAM, следующая пара рядом. Подпись и значение внутри каждой пары имеют стабильную ширину, поэтому строки совпадают по вертикали и горизонтали при обновлении показателей. При отключении всех gauges остаётся настоящий bundle icon MacCleaner.
...
- 74 safety/policy XCTest-тестов, включая persistence-контракт порядка, удаления и восстановления menu bar dashboard-карточек и проверку валидности полного direct-drag порядка.
...
### Быстрый старт Process History

- Уже доступные процессы отображаются сразу, fresh graph sample не ждёт window/disk collectors, а компактные иконки 30 минут/4 часа не имеют подложки.
...
### Thermal Surface: показания и интерполяция

- Поверхность получает только реальные валидные показания. Удалены подстановки SSD = CPU/SoC − 8°C, airflow = CPU/SoC − 12°C, GPU = CPU/SoC и искусственные airflow-копии tdev. Схема/фотореалистичная основа, их пропорции и порядок элементов сохранены.
- Hover компонента показывает `Measured`, реальное значение, HID/SMC source и raw ID. Для logic board выводятся три самых горячих датчика и общее число; остальные доступны в Fans. Отсутствие датчика обозначено `No mapped temperature sensor`, отсутствие RPM — `RPM unavailable`, а не 0.
- Hover волны показывает округлённое `≈…°C · Interpolated` отдельно от текущего показания ближайшего датчика. Сглаживание анимации не меняет отображаемое реальное значение. Геометрия sensor anchors условна внутри соответствующих областей; неизвестные источники не прикрепляются к случайным компонентам.
- Пространственное затухание сохранено, но фон берётся из минимального измеренного значения, а не из произвольной комнатной температуры. Нет заявления о погрешности ±5°C или температуре внешнего корпуса. Шкала Z не обрезает readings выше 95°C.
